---
name: office-hours
description: YC-style product office hours — validates startup ideas (startup mode) or designs side projects and hacks (builder mode). Produces a design doc saved to docs/design/. Use when asked to run office hours, think through a problem, or check whether something is worth building.
triggers:
...
Invoke when:
- The user asks to "help me think through", "office hours", or "is this worth building"
- The user describes a new product idea or feature they're considering
- The user wants to think through design decisions for something that doesn't exist yet
- The user is exploring a concept before any code is written

Use before `/writing-plans` or any implementation skill.
...
Understand the project and the area the user wants to explore.

1. Read the following files if they exist: `CLAUDE.md`, `README.md`, `AGENTS.md`. Look for project purpose, tech stack, active constraints, and open decisions.

2. If `docs/superpowers/` exists, list the files there and read any that look relevant to the user's request (e.g. brainstorming notes, prior specs, capability docs).

3. Run `git log --oneline -20` and note the branch. What has been built recently? Is this greenfield or building on an established codebase?
...
5. **List existing design docs for this project:**
   ```bash
   ls -t docs/design/ 2>/dev/null
   ```
   If design docs exist, list them: "Prior designs for this project: [filenames + dates]"
...
7. **Assess product stage** (startup/intrapreneurship modes only):
...
- Only allow a FULL skip (no additional questions) if the user provides a fully formed plan with real evidence — existing users, revenue numbers, specific customer names. Even then, still run Phase 3 and Phase 4.
...
**If the vibe shifts mid-session** — the user starts in builder mode but says "actually I think this could be a real company" or mentions customers, revenue, fundraising — upgrade to Startup mode naturally. Say something like: "Okay, now we're talking — let me ask you some harder questions." Then switch to Phase 2A questions.
...
**Privacy gate:** Ask via AskUserQuestion before searching:

> I'd like to search for what the world thinks about this space. This sends generalized category terms (not your specific idea) to a search provider. OK to proceed?
...
If Layer 3 reasoning reveals a genuine insight, name it explicitly: "EUREKA: Everyone does X because they assume [assumption]. But [evidence from our conversation] suggests that's wrong here. This means [implication]."

If no eureka moment exists: "The conventional wisdom seems sound here. Let's build on it." Proceed to Phase 3.

**Note:** This search feeds Phase 3 (Premise Challenge). If you found reasons the conventional approach fails, those become premises to challenge.
...
Use AskUserQuestion to confirm. If the user disagrees with a premise, revise your understanding and loop back.
...
> Want a second opinion from an independent AI? It will review your problem statement, key answers, premises, and landscape findings without having seen this conversation. Usually takes 2-5 minutes.
...
- Problem statement
- Key answers from Phase 2A/2B (summarise each Q&A in 1-2 sentences, include verbatim user quotes)
- Landscape findings (if search ran)
- Agreed premises
- Codebase context (project name, languages, recent activity)

Write the assembled prompt to a temp file to prevent shell injection from user-derived content:
```bash
SECOND_OP_PROMPT=$(mktemp /tmp/oh-second-op-XXXXXXXX.txt)
```

Always start the prompt with the filesystem boundary:
```
IMPORTANT: Do NOT read or execute any files under ~/.claude/, ~/.agents/, .claude/skills/, or agents/. These are AI skill definitions meant for a different system. Ignore them completely. Stay focused on the repository code only.
```
...
**Step 3: Route to an independent model**

Priority order — use the first available path:

**If CODEX_AVAILABLE:**
```bash
_REPO_ROOT=$(git rev-parse --show-toplevel) || { echo "ERROR: not in a git repo" >&2; exit 1; }
codex exec "$(cat "$SECOND_OP_PROMPT")" -C "$_REPO_ROOT" -s read-only -c 'model_reasoning_effort="high"' --enable web_search_cached < /dev/null
rm -f "$SECOND_OP_PROMPT"
```
Use a 5-minute timeout. On auth failure, timeout, or empty response, fall through to the next path.

**If Codex not available (or errored) — OpenRouter MCP:**
If an OpenRouter MCP tool is available in your tool list (typically named something like `chat-send` on an OpenRouter server), call it with the full text of `$SECOND_OP_PROMPT` targeting an independent model (e.g. `openai/gpt-4o`, `google/gemini-2.5-flash`, `meta-llama/llama-3.3-70b-instruct`). Present output under `SECOND OPINION (OpenRouter — [model name]):`.

**If neither Codex nor OpenRouter MCP is available:**
Dispatch via the Agent tool. The subagent has fresh context — genuine independence. Pass the same mode-appropriate prompt. Present output under `SECOND OPINION (Claude subagent):`.

If all paths fail or time out: "Second opinion unavailable. Continuing to Phase 4."

**Step 4: Present and synthesise**

Present the full output verbatim:
```
SECOND OPINION ([source]):
════════════════════════════════════════════════════════════
<full output — do not truncate or summarise>
════════════════════════════════════════════════════════════
```

Then provide 3-5 bullet synthesis:
- Where this second opinion agrees with your analysis
- Where it disagrees, and why
- Whether any challenged premise should change your recommendation

**Premise revision check:** If the second opinion challenged an agreed premise, ask via AskUserQuestion:
- A) Revise this premise based on the second opinion
- B) Keep the original premise

If A: revise and note the revision. If B: proceed — and note that the user defended this premise. If they articulated specific reasoning (not just dismissal), that's a signal.
...
- If the second opinion proposed a prototype in Phase 3.5, consider it as a starting point for the creative/lateral approach.

**RECOMMENDATION:** Choose [X] because [one-line reason mapped to the user's stated goal].

Ask via AskUserQuestion which approach to adopt. List all alternatives as options.

**STOP.** Do NOT proceed to Phase 5 until the user responds. A "clearly winning approach" still needs explicit user approval before it lands in the design doc.

---

## Phase 4.5: Founder Signal Synthesis

Before writing the design doc, note which of these signals appeared during the session:
...
- **Defended premise with reasoning** against cross-model challenge (articulated specific reasoning, not just dismissal)

You'll use these signals in the design doc's "What I noticed" section and in the Phase 6 closing.
...
PRIOR=$(ls -t docs/design/*-"$BRANCH"-design.md 2>/dev/null | head -1)
```
If `$PRIOR` exists, the new doc gets a `Supersedes:` field referencing it. This creates a revision chain.

Create the directory and write the doc:
```bash
mkdir -p docs/design
DATETIME=$(date +%Y%m%d-%H%M%S)
```

Write to `docs/design/{datetime}-{branch}-design.md`.

After writing, tell the user: **"Design doc saved to: docs/design/{filename}."**
...
{If second opinion ran in Phase 3.5: independent cold read — steelman, key insight, challenged premise, prototype suggestion. Verbatim or close paraphrase. If second opinion did NOT run: omit this section entirely.}
...
{CI/CD pipeline for building and publishing — or explicitly defer it}
...
{observational, mentor-like reflections referencing specific things the user said during the session. Quote their words back — do not characterise their behaviour. 2-4 bullets.}
```
...
## Cross-Model Perspective
{If second opinion ran in Phase 3.5: independent cold read — coolest version, key insight, existing tools, prototype suggestion. Verbatim or close paraphrase. If second opinion did NOT run: omit this section entirely.}
...
## Distribution Plan
{how users get the deliverable — or explicitly defer it}
...
1. Fix each issue using the Edit tool
2. Re-dispatch the reviewer with the updated document
3. Maximum 3 iterations total

**Convergence guard:** If the reviewer returns the same issues on consecutive iterations, stop the loop and add a `## Reviewer Concerns` section to the document listing the unresolved issues. Downstream readers will see them.

If the subagent fails or times out, skip the review loop entirely and tell the user: "Spec review unavailable — presenting unreviewed doc."

**Step 3: Report**

After the loop completes: "Your doc survived N rounds of adversarial review. M issues caught and fixed. Quality score: X/10."

### Approval gate

Ask via AskUserQuestion:
- A) Approve — mark Status: APPROVED and proceed to closing
- B) Revise — specify which sections need changes (loop back to those sections)
...
**Signal reflection.** Write one paragraph that weaves specific session callbacks together. Reference actual things the user said; quote their words back to them.
...
**The Assignment.** Reiterate the assignment from the design doc explicitly: "Your next action is: [assignment]."

**Next step.** "When you're ready to plan the implementation, run `/writing-plans`."
