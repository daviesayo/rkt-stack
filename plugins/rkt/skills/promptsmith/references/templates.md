...
Full template library for promptsmith. Read the relevant template when the user's task type matches. Do not load all templates at once — only the one you need.
...
| [N — SKKO](#template-n--skko) | Generative/creative outputs (writing, marketing, image, video, voice) |
...
*Situation, Task, Objective, Knowledge, Examples. Use for generative/creative outputs — writing, marketing, image, video, voice — where the prompt's job is to produce content rather than reason about a system. Pairs with bracketed placeholders for any slot that's still ambiguous after intent extraction.*

**Structure rule:** Situation, Task, Objective, and Examples are **fixed** top-level sections. **Knowledge may either be a single section OR decompose into named peer headers** (Brand voice, Constraints, Success criteria, Audience nuances, etc.) when its content is substantial enough that bullets would obscure structure. Pick whichever produces the more readable output for the specific task. Do NOT invent peer headers outside the S/T/O/Examples + decomposed-Knowledge boundary.

```
Situation: [Context the AI needs about who is producing this and why]
Task: [Specific verb + what to produce, in one sentence]
Objective: [What success looks like for the produced content]
Knowledge:
  - [Domain detail, brand voice, audience constraint, length, format]
  - [Anything the AI couldn't infer from the situation alone]
Examples:
  - [1-3 short examples of the desired voice/format if available, OR an explicit "no examples — match Knowledge constraints"]
```

**Example:**
```
Situation: I run a B2B newsletter ([NEWSLETTER_NAME]) for early-stage founders. New subscribers receive a welcome email immediately after signup.
Task: Draft the welcome email body.
Objective: Make the new subscriber feel they made the right choice — set the cadence expectation, hint at the value of the next issue, end with a single reply prompt to surface their context.
Knowledge:
  - Tone: warm, direct, no marketing-speak, no exclamation marks
  - Length: 120-180 words
  - Format: plain text, no headers, single CTA at the end
  - Cadence: weekly, every Tuesday
  - Brand voice: short sentences, concrete nouns, never says "exciting"
Examples:
  - "Hey — Davies here. Thanks for joining [NEWSLETTER_NAME]." (opening voice match)
```

**When to use:**
- Writing tasks (blog posts, emails, landing copy, social posts)
- Marketing content with brand voice constraints
- Image, video, voice prompts where Situation/Knowledge are richer than File/Function/Constraints would be
- Any task where the AI's job is "produce content" not "reason about a system"

**When NOT to use:**
- Code editing (use Template G — File-Scope)
- Agentic tasks (use Template H or M)
- Logic/math/analysis (use Template E — Chain of Thought)
- One-shot simple tasks (use Template A — RTF)

**Bracketed placeholder convention:**
SKKO works well with `[BRACKETED_PLACEHOLDERS]` for anything still ambiguous after intent extraction. Do NOT use placeholders as a substitute for asking — only use them for slots the user explicitly wants to fill in themselves at paste time (e.g., `[NEWSLETTER_NAME]`, `[AUDIENCE]`).
