...
claude
# Then:
/rkt:bootstrap full my-new-thing
```

In Codex, invoke the same skill through the rkt plugin namespace:
...
├── plugins/rkt/                     # canonical cross-tool plugin package
│   ├── .claude-plugin/plugin.json   # Claude plugin manifest
│   ├── .codex-plugin/plugin.json    # Codex plugin manifest
│   ├── skills/                      # /rkt:bootstrap, /rkt:implement, etc.
...
claude --plugin-dir /Users/rocket/Documents/Repositories/rkt-stack/plugins/rkt
...
1. Bump `version` in `plugins/rkt/.claude-plugin/plugin.json` and `plugins/rkt/.codex-plugin/plugin.json`
2. Reinstall/update in the host tool: `claude plugin update rkt@daviesayo-marketplace` or restart/update Codex after marketplace refresh
3. Start a fresh session to load the updated plugin
